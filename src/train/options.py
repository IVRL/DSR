...
loss = parser.add_argument_group('Loss function')
data = parser.add_argument_group('Data')
hw = parser.add_argument_group('Harware')
meta = parser.add_argument_group('Meta-Learning')
...
load_ckp.add_argument('--load-from-mlflow', type=str,
                      help='load model from mlflow')
model.add_argument('--save-checkpoint', type=str,
                   help='save model checkpoint')
model.add_argument('--altitude-embedding', type=int, default=None,
                  help='embedding size for altitude')
...
train.add_argument('--batch-size-val', type=int, default=1,
...
run.add_argument('--report-to-mlflow', action='store_const', const='do',
                 help='report results to mlflow')
resize = run.add_mutually_exclusive_group()
resize.add_argument('--post-resize', action='store_const', const='do',
                 help='resize SR output of X4 model')
resize.add_argument('--pre-resize', action='store_const', const='do',
                 help='resize LR before model')
...
train.add_argument('--seed', type=int, default=6,
                   help='random seed')
train.add_argument('--scale', type=Fraction, required=True,
                   help='upsampling scale')
train.add_argument('--batch-size', type=int, default=16,
                   help='batch size')
train.add_argument('--epochs', type=int, default=1000,
                   help='number of epochs')
loss.add_argument('--loss', type=LossType, default=LossType.L1,
...
train.add_argument('--lr-decay-step', type=int, default=200,
                   help='steps for learning rate decay')
train.add_argument('--lr-decay-steps', type=int, nargs='+',
                   help='steps for learning rate decay')
train.add_argument('--lr-decay-rate', type=float, default=0.5,
...
loss.add_argument('--cobi_use_vgg', action='store_true',
                  help='use VGG feature')

# Dataset specification
data.add_argument('--download-dataset', action='store_true',
                  help='download the dataset')
data.add_argument('--dataset-root', type=str, default=DATASET_ROOT,
                  help='root directory for datasets')
data.add_argument('--dataset-train', nargs='+', default=[DatasetType.Div2KBicubic],
                  type=DatasetType, choices=[DatasetType.Div2KBicubic, DatasetType.Div2KUnknown, DatasetType.DroneSRJPG],
                  help='Training dataset')
data.add_argument('--dataset-val', nargs='+', default=[DatasetType.Div2KBicubic],
                  type=DatasetType, choices=list(DatasetType),
                  help='Validation dataset')
data.add_argument('--dataset-repeat', type=int, default=1,
...
data.add_argument('--patch-size-train', type=int, default=300,
                  help='image patch size for training (HR)')
data.add_argument('--patch-size-val', type=int, default=1000,
...
data.add_argument('--train-altitudes', type=int, nargs='+', default=[10, 20, 30, 40, 50, 70, 80, 100, 120, 140],
                  help='Altitude to train of DroneSR-X7 dataset')
data.add_argument('--val-altitudes', type=int, nargs='+', default=[10, 20, 30, 40, 50, 70, 80, 100, 120, 140],
                  help='Altitude to validate of DroneSR-X7 dataset')
data.add_argument('--dronesr-all-scenes', action='store_true',
                  help='load the whole DroneSR-X7 dataset')
data.add_argument('--synthetic-downsample', action='store_true',
                  help='use synthetic from MATLAB resize as LR')

# Color correction
correction = data.add_mutually_exclusive_group()
correction.add_argument('--white-balance-correction', action='store_true',
                  help='apply white balance correction')
correction.add_argument('--histogram-matching',  action='store_true',
                  help='apply histogram matching')
correction.add_argument('--color-transform', action='store_true',
                  help='apply white balance correction')
correction.add_argument('--color-correction',  action='store_true',
                  help='apply histogram matching')
...
# Meta-learning
meta.add_argument('--meta', action='store_true', help='Enable meta learning')
meta.add_argument('--fomaml', action='store_true', help='Whether to use the first-order approximation of MAML. (FOMAML)')
meta.add_argument('--iterations', type=int, default=10000,
                        help='number of iteration')
meta.add_argument('--send-altitude', action='store_true', help='Send altitude infomation into network')
meta.add_argument('--shots', type=int, default=1,
                        help='number of shots (default: 1)')
meta.add_argument('--fas', '--fast-adaption-steps', type=int, default=5,
                  help='steps per fast adaption (default: 5)')
meta.add_argument('--maml-lr', type=float, default=1e-5,
                    help='learning rate for MAML (default: 0.01)')
meta.add_argument('--learning-rate-change-ratio', type=float, default=1.5)
meta.add_argument('--learning-rate-policy-check-every', type=int, default=50)
meta.add_argument('--learning-rate-slope-range', type=int, default=250)
meta.add_argument('--run-test-every', type=int, default=50)
meta.add_argument('--min-learning-rate', type=float, default=1e-6)
meta.add_argument('--min-iters', type=int, default=250)
...
if args.meta:
    args.batch_size = args.shots * 2
    print("Using meta learning, batch size is disabled.")    
