...
def esrgan(scale, pretrained=False):
...
def bsrgan(scale, pretrained=False):
    model = RRDBNet(3, 3, 64, 23, gc=32)
    if pretrained:
        path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 
                                                     'pretrained','BSRGAN.pth')
        model.load_pretrained(path)
    return model

def bsrnet(scale, pretrained=False):
    model = RRDBNet(3, 3, 64, 23, gc=32)
    if pretrained:
        path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 
                                                     'pretrained','BSRNet.pth')
        model.load_pretrained(path)
    return model
