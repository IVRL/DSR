...
__all__ = [ 'rcan', 'rcan_g10r20f64', 'rcan_altitude']
...
class Upsampler(nn.Sequential):
    def __init__(self, conv, scale: Fraction, n_feats: int, bn=False, act=False, bias=True):
        if scale.denominator != 1:
            raise ValueError('Scale factor should be integer')
...
class RCANAlitiude(nn.Module):
    def __init__(self, n_resgroups, n_resblocks, n_feats, reduction, scale, pretrained=False, map_location=None):
        super(RCANAlitiude, self).__init__()
        self.scale = scale
        ...
        conv=default_conv
        act = nn.ReLU(True)
        url_name = 'g{}r{}f{}x{}'.format(n_resgroups, n_resblocks, n_feats, scale if scale in (2, 3, 4) else 4)
        if url_name in url:
            self.url = url[url_name]
        else:
            self.url = None
        ...
        # define head module
        modules_head = [conv(n_colors+1, n_feats, kernel_size)]
        self.head = nn.Sequential(*modules_head)
    ...
    def load_pretrained(self, map_location=None):
        ...
        pretrained_dict = load_state_dict_from_url(self.url, map_location=map_location, progress=True)
        model_dict = self.state_dict()
        pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict and v.shape==model_dict[k].shape}
        self.load_state_dict(pretrained_dict, strict=False)
        print(f'load from: {self.url}')
...
def rcan_altitude(scale, pretrained=False):
    return RCANAlitiude(n_resgroups=10, n_resblocks=20, n_feats=64, reduction=16,
                scale=scale, pretrained=pretrained)
